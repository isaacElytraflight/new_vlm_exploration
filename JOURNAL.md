...
### Symptom
Default Habitat spawn on Matterport **JmbYfDe2QKZ** put the agent on the **upper** floor (~y=2.67). Stair flights stayed walkable under stock `agent_max_climb=0.2`.

### Root cause
- Shipped `.navmesh` default agent pose is island 1 (upper).
- Island 1 spanned y≈0.5–2.9 (stairs + upper). Ground floor is separate island 0 (~y=0.07).

### Fix
- `sim/scripts/floor_constraint.py` — `select_ground_floor_island` (lowest mean Y among large islands).
- `habitat_engine.py` — on init/reset: rebake navmesh with `HABITAT_AGENT_MAX_CLIMB=0.15` (disconnects stairs), then spawn on ground island.
- Env knobs in `sim/.env.example`: `HABITAT_LOCK_GROUND_FLOOR`, `HABITAT_AGENT_MAX_CLIMB`, `HABITAT_GROUND_MIN_ISLAND_AREA`.

### Verify
- `/usr/bin/python3 -m pytest /workspace/scripts/test_floor_constraint.py -q` → 8/8
- Integration: spawn y 2.67 → 0.07; no geodesic path ground→upper after rebake.

### Closed from backlog
- **JmbYfDe2QKZ spawn floor** (+ cannot climb stairs via navmesh).

### Next backlog (priority)
1. **VLM openness calibration** — scores cluster ~3–4.
2. **Point-cloud mapping** — walls still weak on laser band alone.
3. Secondary: mapper stamp deferrals, costmap TF noise, birdseye per scene, image rebuild after C++ (#9).
...
1. ~~**JmbYfDe2QKZ spawn floor**~~ — done 2026-08-16 (ground island spawn + climb rebake).
...
1. **Coverage vs distance dashboard view** — Extra sim-dashboard panel: realtime graph of **meters traveled** vs **grid-map free-area coverage / Habitat ground-truth floor area** for the active scene.
2. **Scene selector dropdown** — Project Actions control to pick different Habitat environments (reload/restart episode on change).



### Research / quality (from journal, still open)

1. **VLM openness calibration** — Scores still cluster ~3–4; want a usable 1–4 mix (prompt / examples / temperature / prior).
2. **Stronger mapping (point cloud)** — Laser/depth band walls still weak; consider depth **point-cloud** mapping instead of (or in addition to) thin laser band.



### Noise / polish (secondary)

1. `known_pose_mapper` exact-stamp cache deferrals under load.
2. Costmap TF “timestamp earlier than transform cache” (`base_link`).
3. Birdseye / chase-camera orientation check per scene.
4. After durable C++ fixes: rebuild `habitat3-exploration:latest` so **Shutdown** (`compose down`) does not lose `install/` (see note below).
5. Push outstanding `elytra-bridge` commits when ready (nested repo).
...
`Failed to change state for node: collision_monitor. Exception: change_state service is not available!` → `Aborting bringup.`
